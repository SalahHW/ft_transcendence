...
export default class RegisterPopup extends ModalView {
	...
	public render(): void {
		this._contentContainer.innerHTML = /* HTML */ `
			...
			<!-- Formulaire -->
			<form id="popup-container-form-register" class="${UI_THEME.components.form}">
				<div>
					<input
						type="text"
				...
				<div id="popup-container-message-container-register" class="h-6 mt-4">
					<div id="popup-container-message-register" class="${UI_THEME.components.message} opacity-0 invisible transition-all duration-200"></div>
				</div>
	...
	private _attachFormEventListeners(): void {
		const form = document.getElementById("popup-container-form-register") as HTMLFormElement;
	...
	private async _handleSubmit(): Promise<void> {
		const usernameInput = document.getElementById("popup-container-username-register") as HTMLInputElement;
		const emailInput = document.getElementById("popup-container-email-register") as HTMLInputElement;
		const passwordInput = document.getElementById("popup-container-password-register") as HTMLInputElement;
	...
	private _showMessage(message: string, colorClass: string): void {
		const messageElement = document.getElementById("popup-container-message-register");
