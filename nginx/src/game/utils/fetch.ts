...
export async function fetchWithSelfSigned(
  url: string,
  options: RequestInit = {}
): Promise<Response> {
  try {
...
export async function registerCurrentUserForGame(): Promise<{
  id: string;
  username: string;
}> {
...
export async function registerCurrentUserForTournament(): Promise<{
  id: string;
  username: string;
  websocketUrl?: string;
