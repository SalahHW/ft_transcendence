...
import { EnrichedUser } from "../../../services/UserProfileService.js";
...
export class UserProfileView {
  ...
  private static _currentUser: EnrichedUser | null = null;
  ...
  public static async render(container: HTMLElement): Promise<void> {
    ...
    try {
      await this._userProfileService.getEnrichedUserProfile((user) => {
  ...
  private static renderLoadingState(): string {
    return /* HTML */ `
      <div class="flex gap-2 h-full overflow-auto justify-center items-center">
  ...
  private static _renderToDOM(): void {
    ...
    this._container.innerHTML = /* HTML */ `
      <div class="flex gap-2 h-full overflow-auto">
        <div
          id="avatar-container"
          class="relative flex-[1] rounded-lg p-4 aspect-square"
        >
          <img
            id="avatar-img"
            src="${user.avatarUrl}"
            alt="Profile Picture"
            class="w-full h-full object-cover text-white rounded-lg transition-all duration-300"
          />
          <div
            id="avatar-overlay"
            class="absolute inset-4 bg-black/50 rounded-lg flex items-center justify-center opacity-0 transition-opacity duration-300 cursor-pointer pointer-events-none"
          ></div>
          <input
            type="file"
        ...
        <div
          class="flex-[2] rounded-lg p-4 flex flex-col justify-start items-start"
        >
        ...
        <div class="flex flex-col gap-2 p-4 w-12">
          <button
            id="edit-profile-btn"
            class="w-8 h-8 bg-white/10 hover:bg-white/20 rounded-lg flex items-center justify-center text-white transition-colors duration-200"
          >
            <svg
              width="24"
              height="24"
    ...
    try {
      ...
      const editHandler = () => {
        ...
        editButton.innerHTML =
          '<svg width="20" height="20" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="m14.83 4.89l1.34.94l-5.81 8.38H9.02L5.78 9.67l1.34-1.25l2.57 2.4z"/></svg>';
        ...
        const finishEditing = async (save: boolean) => {
          ...
          const usernameInput = usernameWrapper.querySelector(
            'input[name="username"]'
          ) as HTMLInputElement;
          ...
          // Refresh user data with reactive callback
          await this._userProfileService.getEnrichedUserProfile((updatedUser) => {
            ...
            if (save && changesAppliedSuccessfully) {
              avatarImg.src = `${updatedUser.avatarUrl}?t=${new Date().getTime()}`;
            } else {
