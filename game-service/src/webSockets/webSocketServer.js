...
//        // Changed: Store rooms as objects { players: [], ball: null }
...
//
//        // Changed: Access room.players instead of roomPlayers
...
//            // Changed: Check room.players.length
//            if (room.players.length < 2) {
...
//        // Changed: Initialize room as object
...
//            // Changed: Create ball if it doesn't exist
//            if (!room.ball) {
//                room.ball = new Ball(
//                    { playerId: room.players[0].id, playerScore: 0 },
//                    { playerId: room.players[1].id, playerScore: 0 }
...
export class webSocketGameServer {
    ...
    handleNewPlayerConnection(ws) {
        ...
        if (room.players.length === 2) {
            this.notifyPlayersGameStart(room.players, assignedRoom);
            // Changed: Create and initialize ball, broadcast initial ballUpdate
            room.ball = new Ball(
                { playerId: room.players[0].id, playerScore: 0 },
                { playerId: room.players[1].id, playerScore: 0 }
            );
...
broadcastToRoom(roomId, message) {
    ...
    room.players.forEach(({ ws }) => {
        if (ws.readyState === 1) {
            ws.send(json);
            // Changed: Log broadcasts for ballUpdate and sync
            if (message.type === 'ballUpdate' || message.type === 'sync') {
