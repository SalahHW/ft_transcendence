...
export class webSocketGameServer {
  ...
  handlePlayerInput(data, playerId, roomId) {
    ...
    if (msg.type === 'keyDown') {
      ...
      const { positionZ } = msg;
      const now = Date.now();
      const deltaTime = (now - player.lastUpdate) / 1000; // in seconds
      const maxDeltaZ = 20 * deltaTime; // Max movement at speed 20 units/s
      const clampedZ = Math.max(-7.5, Math.min(7.5, positionZ));
      const diff = Math.abs(clampedZ - player.positionZ);
      if (diff <= maxDeltaZ + 0.01) { // Allow small floating-point tolerance
        player.positionZ = Number(clampedZ.toFixed(3));
        //console.log(`Updated player ${playerId} positionZ to ${player.positionZ} from client`);
      } else {
        //console.log(`Rejected player ${playerId} positionZ=${positionZ}, diff=${diff}, maxDeltaZ=${maxDeltaZ}`);
      }
  ...
  startGameLoop() {
    ...
    const SYNC_INTERVAL = 10; // Sync every 50ms
    ...
    const update = () => {
      ...
      this.players.forEach((player, playerId) => {
        ...
        const speed = 20; // units/s, matches client
